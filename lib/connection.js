...
var Connection = module.exports = function(options) {
  ...
  if (!refreshFn && this.oauth2.clientId && !this.oauth2.privateKey) {
    refreshFn = oauthRefreshFn;
  } else if (!refreshFn && this.oauth2.clientId && this.oauth2.privateKey) {
      refreshFn = oauthJwtBearerFn;
  }
  if (refreshFn) {
...
/** @private **/
function oauthJwtBearerFn(conn, callback) {
    conn.oauth2.jwtBearerToken(conn.username, function(err) {
        if (err) { return callback(err); }
        var userInfo = parseIdUrl(res.id);
        conn.initialize({
            instanceUrl : res.instance_url,
