 ...
 * @param {String} [options.privateKey] - OAuth2 private key (This is only required for JWT Bearer Token Flow).
...
_.extend(OAuth2.prototype, /** @lends OAuth2.prototype **/ {
   ...
   * @returns {Promise.<TokenResponse>}
   */
  jwtBearerToken : function(username, callback) {
    var options = {
        issuer: this.clientId,
        audience: this.loginUrl,
        expiresIn: 3,
        algorithm:'RS256'
    }
    var token = jwt.sign({ prn: username }, this.privateKey, options);
    var params = {
